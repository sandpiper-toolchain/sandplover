...
ch_xarr = [
    xr.DataArray(
        data=np.reshape(
            chmask[i].mask.data, (1, chmask[i].mask.shape[0], chmask[i].mask.shape[1])
        ),
        coords=coords,
        dims=dims,
    )
...
def test_check_input_single_mask_error():
    ...
    with pytest.raises(TypeError, match=r"Cannot input a Mask .*"):
        chmap, landmap, basevalues, time_window, dim0 = check_inputs(
            chmask[0], basevalues_idx=[0], window_idx=1, landmap=landmask[0]
        )
...
def test_check_xarrays():
    ...
    # call checker function
    chmap, landmap, basevalues, time_window, dim0 = check_inputs(
        ch_xarr, basevalues_idx=[0], window_idx=1, landmap=land_xarr
    )
    # assert types
    assert dim0 == "time"
    assert isinstance(chmap, xr.DataArray) is True
...
def test_check_list_ndarrays():
    ...
    # call checker function
    chmap, landmap, basevalues, time_window, dim0 = check_inputs(
        ch_arr_list, basevalues_idx=[0], window_idx=1, landmap=land_arr_list
    )
    # assert types
    assert dim0 == "time"
    assert isinstance(chmap, xr.DataArray) is True
...
def test_check_ndarrays():
    ...
    # call checker function
    chmap, landmap, basevalues, time_window, dim0 = check_inputs(
        ch_arr, basevalues_idx=[0], window_idx=1, landmap=land_arr
    )
    # assert types
    assert dim0 == "time"
    assert isinstance(chmap, xr.DataArray) is True
...
def test_check_input_notbinary_landmap():
    ...
    with pytest.raises(ValueError):
        check_inputs(ch_bin, basevalues_idx=[0], window_idx=1, landmap=land_nonbin)


def test_check_input_invalid_chmap():
    """Test that an invalid channel map input will throw an error."""
    with pytest.raises(TypeError):
        check_inputs("invalid", basevalues_idx=[0], window_idx=1, landmap=landmask)


def test_check_input_invalid_landmap():
    """Test that an invalid landmap will throw an error."""
    with pytest.raises(TypeError):
        check_inputs(chmask, basevalues_idx=[0], window_idx=1, landmap="invalid")
...
def test_check_input_diff_shapes():
    ...
    with pytest.raises(ValueError):
        check_inputs(
            chmask, basevalues_idx=[0], window_idx=1, landmap=np.ones((3, 3, 3))
        )
...
def test_check_input_1dlandmask():
    ...
    with pytest.raises(TypeError):
        check_inputs(chmask, basevalues_idx=[0], window_idx=1, landmap=np.ones((10, 1)))
...
def test_check_input_invalid_list():
    """Test a wrong list."""
    with pytest.raises(TypeError):
        check_inputs(["str", 5, 1.0], basevalues_idx=[0], window_idx=1)
...
def test_check_input_castlandmap():
    ...
    chmap, landmap, bv, tw, dim0 = check_inputs(
        chmask, basevalues_idx=[0], window_idx=1, landmap=landmask[0].mask[:, :]
    )
...
def test_planform_olap():
    ...
    assert pytest.approx(ophi.values) == np.array(
        [[1.0, 0.66666667, 0.33333333, 0.0, -0.33333333]]
    )
...
def test_reworking():
    ...
    assert pytest.approx(fr.values) == np.array(
...
def test_channel_abandon():
    ...
    assert np.all(ch_abandon == np.array([[0.0, 0.25, 0.5, 0.75, 1.0]]))
