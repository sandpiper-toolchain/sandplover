...
def over_coordinates(ax_lims):
    tri_low = np.array([[0, 0], [0 - 10, 0.5], [0, 1]])
    tri_high = np.array([[N, 0], [N + 10, 0.5], [N, 1]])
...
def show_an_info(Info, ax):
    ...
    if not Info.norm:
        ...
        ax.add_patch(
            mpl.patches.Polygon(
                tri_low, closed=True, color=Info.cmap._rgba_over, clip_on=False
            )
        )
        ax.add_patch(
            mpl.patches.Polygon(
                tri_high, closed=True, color=Info.cmap._rgba_over, clip_on=False
            )
        )
